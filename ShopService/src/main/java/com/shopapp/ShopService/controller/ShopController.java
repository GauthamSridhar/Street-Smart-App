...
public class ShopController {
  ...
  public ResponseEntity<ShopResponse> updateShop(
      @PathVariable UUID shopId, @Valid @RequestBody UpdateShopRequest request) {
  ...
  @GetMapping("/search")
  public org.springframework.data.domain.Page<ShopResponse> search(
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int size,
      @RequestParam(defaultValue = "") String q,
      @RequestParam(defaultValue = "") String category) {
    return shopService.search(page, size, q, category);
  }

  // The legacy list endpoint is bounded; use /search for pagination.
