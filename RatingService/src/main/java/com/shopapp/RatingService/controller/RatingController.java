...
public class RatingController {
  ...
  public ResponseEntity<List<RatingResponseDTO>> getShopRatings(
      @PathVariable UUID shopId,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "20") int size) {
    log.info("Fetching ratings for Shop ID: {}", shopId);
    List<RatingResponseDTO> ratings = ratingService.getShopRatingsPage(shopId, page, size);
    return ResponseEntity.ok(ratings);
  }

  @GetMapping("/summary/{shopId}")
  public java.util.Map<String, Number> summary(@PathVariable UUID shopId) {
    return ratingService.summary(shopId);
  }

  // Get the number of reviews for a visible shop.
  ...
  public record ResolveRequest(
      @jakarta.validation.constraints.NotBlank
          @jakarta.validation.constraints.Pattern(regexp = "DISMISS|REMOVE")
          String action) {}

  @PostMapping("/{ratingId}/reports")
  public java.util.Map<String, Object> report(
      @PathVariable UUID ratingId, @Valid @RequestBody ReportRequest request) {
    return ratingService.report(ratingId, request.reason());
  }

  @GetMapping("/reports")
  public List<java.util.Map<String, Object>> reports(
      @RequestParam(defaultValue = "OPEN") String status) {
    return ratingService.reports(status);
  }

  @PostMapping("/reports/{reportId}/resolve")
  @ResponseStatus(org.springframework.http.HttpStatus.NO_CONTENT)
  public void resolve(@PathVariable UUID reportId, @Valid @RequestBody ResolveRequest request) {
    ratingService.resolveReport(reportId, request.action());
  }
}
